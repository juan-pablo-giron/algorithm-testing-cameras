...
PATH_inputs=$(kdialog --title "Select the folder where are the inputs of your circuit" --getexistingdirectory *);
if [ "$?" -ne 0 ]; then
  kdialog --error "Simulation Aborted"
  cd $PATH_scriptMatlab
  exit 1
...
#python sort_data_DVS_pixel_UNIX.py "$PATH_sim_output_matlab" "$PATH_folder_simulation" "$name_simulation" "$name_folder_output_Spectre" "$number_bits"
