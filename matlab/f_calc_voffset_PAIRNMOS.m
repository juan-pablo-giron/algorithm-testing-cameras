function [sok_voffset,Voffset_total] = f_calc_voffset_PAIRNMOS(Voffsetmin,Voffsetmax,Wpair,Lpair,Wmirror,Lmirror,gmid_pair,gmid_mirror)
    ...
    %mismatch
    sigma_vt_2 = (Avtnmos^2)/(Wpair*Lpair);
    sigma_beta_2 = (Abetanmos^2)/(Wpair*Lpair);
    Voffset_pair = sqrt(sigma_vt_2 + sigma_beta_2/gmid_pair^2);

    % mirror current

    % mismatch 
    sigma_vt_2 = (Avtpmos^2)/(Wmirror*Lmirror);
    sigma_beta_2 = (Abetapmos^2)/(Wmirror*Lmirror);
