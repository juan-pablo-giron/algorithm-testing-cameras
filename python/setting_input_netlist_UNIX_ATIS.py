...
l_nodevoltageNames = []     # Save the name of the node voltage DVS
l_nodecurrentNames = []     # Save the name of the nodes current name DVS
l_nodevoltageNames_A = []     # Save the name of the node voltage ATIS
l_nodecurrentNames_A = []     # Save the name of the nodes current name ATIS
...
x = 0
while x <quant_pixels:
    nodeVoltageName = name_n_VoltageATIS+str(x)
    nodeCurrentName = name_n_CurrentATIS+str(x)
    l_nodevoltageNames_A.append(nodeVoltageName)
    l_nodecurrentNames_A.append(nodeCurrentName)
    x = x + 1

len_nodeVoltageName_A = len(l_nodevoltageNames_A)
...
while x < len_netlist:
    ...
    if (find_tran != -1):
        ...
        while (j<len_lst_tmp):
            ...
            if (find_T != -1):
                lst_tmp[j]='stop=%1.10f'%(T_Rst+float(T))
                string=' '.join(lst_tmp)
                l_netlist[x]=string
                j=len_lst_tmp
            else:
    ...
    if (find_nameVoltage != -1):
         ...
         ## ATIS
             ...
             find_nameVoltage_A = string.find(name_n_VoltageATIS)
             if (find_nameVoltage_A != -1):
                 i=i+1
                 #print 'x = %d'%x+'  lst[X]'+string
                 lst_tmp=string.split(' ')
                 lst_tmp=[w.replace('(','') for w in lst_tmp]
                 #print lst_tmp
                 voltage_name=lst_tmp[1] # here the name node voltage
                 index=l_nodevoltageNames_A.index(voltage_name)
                 #print index
                 j=x+1
                 while j<len_netlist:
                     string=l_netlist[j]
                 ...
                 #print 'nxt_inst %d' %nxt_ins
                 del l_netlist[x:nxt_ins]
                 line1 = l_nodecurrentNames_A[index]+' ('+l_nodevoltageNames_A[index]+' 0) isource \\'
                 line2 = '      file='+'"'+ PATH_folder_input+name_Signalsinput+'_'+str(index)+ext_input+'"'+' \\'  
                 line3 = '      type=pwl delay=T_Rst edgetype=halfsine scale=1 stretch=1 pwlperiod=T'
                 l_netlist.insert(x,line1)
                 l_netlist.insert(x+1,line2)
